.. FarmSubsidy.org documentation master file, created by
   sphinx-quickstart on Mon Mar 10 13:23:20 2014.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

FarmSubsidy.org - Developer Documentation
=========================================

This documentation is intended for developers who want to help out with the 
`FarmSubsidy.org <http://farmsubsidy.openspending.org>`_ project which aims
to bring transparency to EU farm subsidies and is run by the `OpenSpending <http://openspending.org/>`_ 
community.

Introduction
------------

The ``website`` section is 
describing the structure of the website and the corresponding source code
on `GitHub <https://github.com/openspending/Farm-Subsidy>`_. Scrapers for
the data are hosted in a separate `GitHub repository <https://github.com/openspending/farmsubsidy-scrapers>`_.
People who want to help out writing scrapers for the different countries
can find information in the ``scraper`` section of these docs.

.. note::
   Current work in progress, for remarks or questions
   please contact `holgerd77 <https://github.com/holgerd77>`_  on Github (March 10th, 2014).

User Manual:

.. toctree::
   :maxdepth: 2
   
   website
   scraper
   

Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`

